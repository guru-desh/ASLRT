...
* Navigate to either `projects/Kinect`, `projects/Mediapipe` or `projects/AlphaPose` depending on which pose estimator you wish to use.
...
* Open a terminal at `projects/Kinect`, `projects/Mediapipe` or `projects/AlphaPose` and execute `python3 driver.py`. Example: `python3 driver.py --test_type cross_val --train_iters 25 50 75 100 120 140 160 180 200 220 --hmm_insertion_penalty 0 --cross_val_method leave_one_user_out --n_splits 4 --cv_parallel --parallel_jobs 5 --prepare_data` 
* This will first prepare the HTK files for HMMs on all user data located at the base directory `"features_dir"` by using user independent cross validation of 4 folds on 5 parallel processes. The specifics of some common options are given in the next section. Please look at `main/src/main.py` for the complete list of arguments that can be used with the pipeline.
...
* Data List Parameters
  ...
  * [**IMP**] `driver.py --parallel_jobs`: If `cv_parallel` is set then this is the number of processes to use. It is recommended to make this equal to the number of users or splits you have. 
...
* SBHMM Training Parameters
  ...
  * [**IMP**] `driver.py --train_sbhmm`: `True` if you want to train SBHMMs.
  ...
  * `driver.py --pca_components`: Number of PCA components that should be used if you are PCA'ing features after the classifier training.
...
* Simple Verification: Refers to using HVITE for alignment and using a static threshold for accepting and rejecting phrases.
* Verification Zahoor: Refers to the verification algorithm proposed by Zahoor. You can find more information here: https://dl.acm.org/doi/pdf/10.1145/2070481.2070532
